...
*/
let synth = null;
let oneOnly = 0;  // for debugging draw only one part
let fadeW = 1.5;
...
function keyStart(key = 60, v = 100, ch = 0) {
    //console.log("keyStart", key, v);
    synth.noteOn(ch, key, v);
}

function keyStop(key = 60, ch = 0) {
...
class Note {
    ...
    setState(state, v) {
        if (this.state != state) {
            this.state = state;
            if (state == "on") {
                keyStart(this.mnote, v, this.ch);
            } else {
                keyStop(this.mnote, this.ch);
            }
        }
    }
}
...
class Part {
    constructor(i, cx, cy, patternName) {
        console.log("Part(", i, ")");
        this.i = i;
        this.T = 10;
        this.t = 0;
        ...
        this.cy = cy;
        this.vel = 100;
        this.setNotes(patternName);
    }

    setNotes(name) {
        console.log("Part.setNotes", name);
        ...
        for (let i = 0; i < numSteps; i++) {
            ...
            let ch = 0;
            if (typeof n == "number") {
                this.notes.push(new Note(i / numSteps, MNOTE + n, ch));
            }
            else {
                ch = n[1];
                n = n[0];
    ...
    getNoteGraphic(i) {
        ...
        let x, y;
        if (0) {
            x = this.cx + this.r * Math.sin(th);
            y = this.cy - this.r * Math.cos(th);
        }
        else {
            x = this.cx - this.r * Math.sin(th);
            y = this.cy + this.r * Math.cos(th);
...
class Player {
    ...
    constructor(canvasId) {
        ...
        for (let i = 0; i < numParts; i++) {
            let part = new Part(i - 1, this.cx, this.cy, this.patternName);
            part.setSpeed(Math.pow(2, -i));
            this.parts.push(part);
        }
        this.reset();
        synth.setProgram(0, 0);     // piano
        synth.setProgram(1, 116);   // taiko
        synth.setProgram(2, 115);   // woodblocks
    }

    setNotes(patternName) {
        console.log("Player.setNotes", patternName);
    ...
    checkForNewParts() {
        ...
        if (this.speedFactor < 1) {
            ...
            if (lgs < -3) {
                ...
                let i = part0.i - 1;
                let part = new Part(i, this.cx, this.cy, this.patternName);
